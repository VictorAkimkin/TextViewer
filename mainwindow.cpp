...
void MainWindow::on_action_triggered()
{
    ...
    if (str!="")
    {
        ResetAll();// сначала закрываем открытый файл и приводит все в исходное состояние
        fname=str;// затем меняем имя фала
        FileOpening ();
    }
}
// Процедура открытия файла используется и в конструкторе, и при выборе пункта меню
void MainWindow::FileOpening ()
{
    if (fname=="") return;
    ...
    else QMessageBox::warning(this, "Ошибка", "Не удалось открыть выбранный файл: "+fname, QMessageBox::Ok);
}
// Закрываем файл
...
{
    QMessageBox::about(this, "О программе", "Программа для просмотра шестнадцатеричного кода файлов\n(c) Акимкин Виктор");
}
...
// Поменять режим просмотра на строковый без учета спецсимволов
...
// Поменять режим просмотра на строковый с учетом спецсимволов
...
void MainWindow::removeSpecialCharacters(QString &str)
{
    for (auto& elem :str)
    {
        if (elem==QChar::Null || elem==QChar::Tabulation || elem==QChar::LineFeed || elem==QChar::FormFeed ||
            elem==QChar::CarriageReturn || elem==QChar::Nbsp || elem==QChar::SoftHyphen ||
...
void MainWindow::searchInFile(int_least64_t addr, QString value,  SearchForm::Data_Type type,bool char_register, bool equal_or_not)
{
    ...
    if (DoSearch(search_data, file))
    {
        ...
        if (ui->action_5->isChecked()) return;// не подсвечивать текст в формате просмотра с учетом спец символов
