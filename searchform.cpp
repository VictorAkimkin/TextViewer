...
SearchForm::SearchForm(QWidget *parent,int_least64_t addr) :
    QWidget(parent),
    ui(new Ui::SearchForm),Mte (new MyTextEdit)
{
    //this->setAttribute(Qt::WA_DeleteOnClose);
    ui->setupUi(this);
    ui->plainTextEdit->setPlainText(QString::number(addr));
 //   ui->verticalLayout->addWidget(Mte);
...
{ //return;
   QTextCursor cur=ui->plainTextEdit_2->textCursor();
   int cursor_pos= cur.position();
   int str_lenght = ui->plainTextEdit_2->toPlainText().length();
    ...
    {
        QString str=checkTextStringMax(ui->plainTextEdit_2);
        if (str!="") ui->plainTextEdit_2->setPlainText(str);
    }
    if (str_lenght==ui->plainTextEdit_2->toPlainText().length());
...
void SearchForm::checkFloatMax(QPlainTextEdit *TextBox)
{
    ...
    if (val< 0.0000000001 &&val)
    {
        TextBox->setPlainText (QString::number( 0.0000000001,'f',10));
        return;
    }
    new_str=QString::number(val,'f',precision);
    if (new_str!=old_str || precision>10)
    {
        if (old_str[lenght-2]=='.')
        {
           old_str[lenght-1]='0';
...
QString SearchForm::checkTextStringMax(QPlainTextEdit *TextBox)
{
    ...
    for (auto &elem:str)
        if (elem==QChar::Null || elem==QChar::Tabulation || elem==QChar::LineFeed || elem==QChar::FormFeed ||
            ...
            elem==QChar::LastValidCodePoint || elem<' ') {elem=' ';isChanged=true;}
      if (str.length()> MAX_TEXT_LENGHT) return str.remove(str.length()-1,1);
      if (isChanged)return str;
    return "";
}
